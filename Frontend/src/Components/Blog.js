...
//         console.log("iiiiiiiiiiiiddddddddddd",id);
//         return <Link to={`/preview/?_id=${id}`}>{params.value}</Link>;
//       },
//     },
...
const Blog = () => {
  ...
  const columnDefs = [
    { headerName: "ID", field: "_id", width: 100 }, // Use _id instead of id if that's how your data is structured
    {
      headerName: "Title",
      field: "title",
      flex: 1,
      cellRenderer: (params) => {
        const { _id } = params.data; // Use _id instead of id if that's how your data is structured
        ...
        <Col>
          <header className="text-center mb-4">
            <div className="d-flex justify-content-between align-items-center">
              ...
              {userRole !== "Admin" ? (
                <Button variant="primary" onClick={handleShowAddModal} style={{ marginRight: "18px", padding: "5px 8px" }}>
                  Add Blog
                </Button>
              ) : (
                ""
